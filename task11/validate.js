...
function checkn(){
    ...
    if(val.length>=5 && val.length<26){
        ...
        document.getElementById("00").innerHTML="enter Valid name (Name should be from 5 - 25 characters)";
    }
}
function pwd(){
...
function validmail(){
    ...
    if(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(mail)){
        ...
        document.getElementById("2").innerHTML="enter valid email-id(e.g.,example@domain.extension)";
    }
}
...
function validno(){
   var num=document.getElementsByName("number")[0].value;
    if(/^\d{10}$/.test(num)){
        ...
        document.getElementById("3").innerHTML="Enter Valid Phone number";
    } 
}
